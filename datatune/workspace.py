...
class Workspace:
    ...
    The Workspace class is responsible for creating, deleting, and listing both datasets and views.
    ...
    def __init__(self, uri: str, token: str):
        ...
        self.api: API = API(api_key=token)
        self.user, self.workspace_name = self._parse_uri(uri)

    def add_credentials(self, config: Dict[str, str]) -> None:
        """
        Adds cloud storage credentials to the workspace for data handling.
        
        Args:
            config (Dict[str, str]): Configuration dictionary containing credentials for cloud storage.
                Expected keys:
                - 'aws_access_key_id'
    ...
    def add_dataset(self, dataset_id: str, path: str, is_local: bool = False) -> Dataset:
        """
        ...
        return Dataset(dataset_id, api=self.api, workspace=self)
    ...
    def load_dataset(self, dataset_id: str) -> Dataset:
        ...
        Args:
            dataset_id (str): Unique identifier of the dataset to load.
    ...
    def delete_dataset(self, dataset_id: str) -> 'Workspace':
        """
        Deletes a dataset from the workspace.

        Args:
            dataset_id (str): Unique identifier of the dataset to delete.
    ...
    def list_datasets(self) -> List[Dataset]:
        ...
        datasets = [Dataset(ds['dataset_id'], api=self.api, workspace=self) for ds in response.get('datasets', [])]
        return datasets
    ...
    def load_view(self, view_name: str) -> View:
        """
    ...
    def delete_view(self, view_name: str) -> 'Workspace':
        """
    ...
    def _parse_uri(self, uri: str) -> (str, str):
        """
        Parses the URI to extract the user and workspace name.

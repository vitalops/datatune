...
def update_df_with_llm_output(df: pd.DataFrame, expected_fields=None) -> pd.DataFrame:
    ...
    result_df = result_df.drop(
        columns=[
            col
            for col in [_SERIALIZED_INPUT, _MAP_PROMPT, __LLM_OUTPUT__]
...
class Map:
    ...
    def __call__(self, llm: Callable, df: Dict):
        ...
        output_cols = [
            ...
            if col not in [_SERIALIZED_INPUT, _MAP_PROMPT, __LLM_OUTPUT__]
        ]
        output_cols.extend(expected_fields)
        meta = pd.DataFrame(columns=output_cols)

        update_func = partial(
            update_df_with_llm_output, expected_fields=expected_fields
        )
